...
class LimineInstaller:
    ...
    def _create_config(self, config_dir: Path) -> bool:
        ...
        if self.luks_uuid:
            # Use luks-<UUID> format to match fstab generated by genfstab/Calamares
            # allow-discards enables TRIM support for SSDs through LUKS
            mapper_name = f"luks-{self.luks_uuid}"
            cmdline_parts.append(f"cryptdevice=UUID={self.luks_uuid}:{mapper_name}:allow-discards")
            cmdline_parts.append(f"root=/dev/mapper/{mapper_name}")
        else:
        ...
        # Solution: Kernels will be copied to ESP AFTER mkinitcpio regeneration
        # in configure_mkinitcpio() to ensure correct initramfs with LUKS hooks
        kernel_uri = "boot():"
        module_uri = "boot():"
